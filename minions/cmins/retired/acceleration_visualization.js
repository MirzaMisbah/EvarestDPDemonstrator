window.addEventListener('load', function (event) {
    var accelerationSensorsMinion = function (_divId, _dataChannel) {
        'use strict';
        ...
        var updateUI = function (dataPoint) {
            ...
            if (showing) {
                ...
                var newNode = document.createTextNode('Acceleration value: ' + currentData);
                h3.replaceChild(newNode, textNode);
                textNode = newNode;
                groups.acc.value = currentData;
            } else {
                clearDiv();
            }
        ...
        function build_chart() {
            var build_acceleration = function () {
                ...
                g.append("g")
                    ...
                    .append("path")
                    .datum(groups.acc.data)
                    .attr("class", "line")
                    .style('stroke', groups.acc.color)
                    .transition()
                    .duration(duration)
                    .ease(d3.easeLinear)
                    .on("start", tick);
    ...
    app.minionLoader.storeNewMinion('Acceleration Motion', 'communicator', accelerationSensorsMinion.toLocaleString());
});