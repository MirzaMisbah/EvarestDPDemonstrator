...
define(function (require) {
     ...
     */
    function CrossDataStorageHub(thirdPartyURL) {
        this.origin = location.href;
        this.dataStorage = new DataStorage();
        this.thirdPartyURL = thirdPartyURL;
        this.auth0Configurator = new Auth0Configurator('kMlSIl3Itqt6mQetzGXES6biAVFei6k8', 'app-iss.eu.auth0.com');
        this.hostSettings = {};
        this.keys = [];
    }
     ...
     */
    CrossDataStorageHub.prototype.connect = function (createHandler, readHandler, updateHandler, deleteHandler, keys = ['type', 'unit', 'timestamp', 'applicationID', 'values', 'userID']) {
        this.keys = keys;
        if (createHandler)
            this.setCreateHandler(createHandler);
        else
            this.setCreateHandler();

        if (readHandler)
            this.setReadHandler(readHandler);
        else
            this.setReadHandler();

        if (updateHandler)
            this.setUpdateHandler(updateHandler);
        else
    ...
    privateMethods.initializePostApi = function (_this) {
        const handleRequest = function (event) {
            ...
            const access_token = dataObject.access_token;
            verifiers.verifyUserToken(_this, {
                id_token: user_token, access_token : access_token, multiUser: dataObject.query.multiUser, success: function (userID, profile) {
    ...
    verifiers.verifyOrigin = function (_this, origin, method, userID) {
        ...
        return _this.getSettingsOfHost(origin, userID)
            .catch(function (error) {
                const initialSetting = confirm('Do you want this application to work on your personal data?');
                return _this.dataStorage.insertNewHost(origin, userID, initialSetting)
                    .then(function () {
                        console.log('here');
                        return Promise.resolve(_this.getSettingsOfHost(origin, userID));
                    });
            })
                ...
                if (hosts.indexOf(id) > -1) {
                    if (_this.hostSettings[id]) {
                        ...
                        return _this.getSettingsOfHost(origin, userID)
                            .then(function (settings) {
                                return {
                                    status: settings.methods[method],
                                    message: 'Method allowed status: ' + settings.methods[method]
                                };
                            })
                    }
                }
    ...
    verifiers.verifyCreateObject = function (_this, object) {
        ...
        var reference = keys.length > _this.keys.length ? _this.keys : keys;
        var compare = keys.length > _this.keys.length ? keys : _this.keys;
        for (let i = 0; i < reference.length; i++) {
            if (compare.indexOf(reference[i]) > -1) {
                continue;
            } else {
                return false;
            }
    ...
    verifiers.verifyUserToken = function (_this, params) {
        ...
        /*if (!params.access_token) {
            params.error('Please Specify access_token to verify');
            return;
        }*/
            ...
            if (err) {
                ...
                if (params.multiUser){
                    ...
                    var body = { method: 'GET',
                        ...
                        mode: 'cors',
                        cache: 'default' };
                    var processMultiuser = function (json) {
                        if (json.app_metadata.data_requests_out){
                            var reqs = [];
                            for (var i = 0; i < json.app_metadata.data_requests_out.length; i++){
                                if (json.app_metadata.data_requests_out[i].status == 'accepted') {
                                    reqs.push(json.app_metadata.data_requests_out[i].email);
                                }
                            }
                            for (var i = 0; i < params.multiUser.length; i++) {
                                if (reqs.indexOf(params.multiUser[i]) < 0) {
                                    params.error('No access rights for this user: ' + params.multiUser[i]);
                                    return;
                                }
                            }
                            ...
                            params.error('No access rights for other user');
                            return;
                        }
                    };
                    if (!profile.app_metadata) {
                        fetch(_this.thirdPartyURL + '/get_user_credentials', body).then(function (response) {
                            if(response.ok)
                                return response.json();
                            else
                                throw Error('Could not access server ' + _this.thirdPartyURL);
                        }).then(function (json) {
                            // Display user information
                            processMultiuser(json);
                        });
                    } else {
                        processMultiuser(profile);
                    }
    ...
    eventHandlerMethods.defaultCreateHandler = function (_this, event, dataObject, userID) {
        ...
        for (let i = 0; i < dataObject.query.dataObjects.length; i++) {
            ...
            if (!dataObject.query.dataObjects[i].userID){
    ...
    eventHandlerMethods.defaultReadHandler = function (_this, event, dataObject, userID) {
        ...
        const response = {status: 'success', dataObjects : {}};
        if (!dataObject.query.multiUser){
            ...
            var readStepByStep = function (users) {
                if (users.length > 0) {
                    var currentUser = users.pop();
                    _this.dataStorage.readData(queryId, currentUser)
                        .then(function (res) {
                            response.dataObjects[currentUser] = res.dataObjects;
                            readStepByStep(users);
                        });
                } else {
                    response.message = 'Successfully fetched data';
                    privateMethods.makeResponse(event, dataObject, response);
            ...
            readStepByStep(dataObject.query.multiUser);
        }
    ...
    eventHandlerMethods.defaultUpdateHandler = function (_this, event, dataObject, userID) {
        ...
        if (dataObject.query.multiUser) {
            if (dataObject.query.multiUser.length === 1) {
                userID = dataObject.query.multiUser[0];
            }
        }
